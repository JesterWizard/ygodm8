...
void sub_8020968(void *, u16, u16);
void ConvertU16ToDigitBuffer(u16, u8);

static void CopyCustomCardArtDataToBuffers(void) {
  u8 i;
  u8 ok = 0x48;

  sub_800E08C((void *)GetBigCardArtAsset(gCardInfo.id), gUnk_8E01364 + 32);
  CpuFill16(0, gUnk_8E01364, 64);
  CpuCopy32(GetBigCardPaletteAsset(gCardInfo.id), gUnk_8E01368, 128);
  *gUnk_8E01368 = 0;
  for (i = 0; i < 10; i++)
    CpuCopy32(gUnk_8936130[i], gUnk_8E0136C + (10 * i + ok + i * 4), 20);
}

static void CopyCustomAttributeIcon(void) {
...
static void CopyCustomTypeIcon(void) {
  u8 type;
  u8 sb;
...
static void CopyCustomAtkDigits(void) {
  u8 i;
...
static void CopyCustomDefDigits(void) {
  u8 i;
...
static void CopyCustomCardName(void) {
  u8 pos;
  bool32 abbreviate;
  u32 buffer[16];
...
static void ResetCustomBigCardFrame(void) {
  u16 i;
  ...
  CpuCopy32(gUnk_8E137C4[gCardInfo.color], gUnk_8E01368 + 0x40, 40);
}

static void CopyCustomStarIcons(void) {
...
LYN_REPLACE_CHECK(PrintCard);
APPEND_TEXT void PrintCard__Replacement(void) {
  ResetCustomBigCardFrame();
  CopyCustomCardArtDataToBuffers();
  CopyCustomAttributeIcon();
  CopyCustomTypeIcon();
  CopyCustomStarIcons();
  CopyCustomAtkDigits();
  CopyCustomDefDigits();

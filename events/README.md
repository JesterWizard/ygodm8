...
`make event-compile` compiles `events/scripts/*.c` when present. YAML is only an extraction/interchange format now; run `make event-extract` if you need to recreate it from `baserom.gba`.
...
After editing C macro files, compile replacements:

#include "event_macros.h"
#include "event_object_slots.h"
#include "overworld.h"
#include "constants/music_ids.h"

/* map_26_state_06: map 26 state 6 */

#define LOCALID_MAI            1
#define LOCALID_ARKANA         2
#define LOCALID_NEO_GHOUL_1    3
#define LOCALID_NEO_GHOUL_2    4
#define LOCALID_RICH_HUSBAND   5
#define LOCALID_RICHE_WIFE     6

LOAD_SPRITE(LOCALID_MAI, SPRITE_MAI)
LOAD_SPRITE(LOCALID_ARKANA, SPRITE_ARKANA)
LOAD_SPRITE(LOCALID_NEO_GHOUL_1, SPRITE_NEO_GHOUL)
LOAD_SPRITE(LOCALID_NEO_GHOUL_2, SPRITE_NEO_GHOUL)
LOAD_SPRITE(LOCALID_RICH_HUSBAND, SPRITE_RICH_HUSBAND)
LOAD_SPRITE(LOCALID_RICHE_WIFE, SPRITE_RICHE_WIFE)
PLAY_MUSIC(MUSIC_407)
DELAY(16)
TALK(PORTRAIT_MAI, EXPRESSION_MAI_ANGRY, PORTRAIT_LEFT,
    "Harpie Lady!\n\n"
)
PLAY_MUSIC(MUSIC_333)
SHOW_OVERWORLD_GRAPHIC(GRAPHIC_HARPIE_LADY_SISTERS_FULL_ART)
DELAY(16)
TALK(PORTRAIT_MAI, EXPRESSION_MAI_ANGRY, PORTRAIT_LEFT,
    "Harpies' Triangle Ecstasy!\n\n"
)
PLAY_MUSIC(MUSIC_341)
SHOW_OVERWORLD_GRAPHIC(GRAPHIC_HARPIE_LADY_SISTERS_ATTACK)
DELAY(16)
PLAY_MUSIC(MUSIC_284)
SLIDE_OBJECT(LOCALID_NEO_GHOUL_1, 1, 8)
PLAY_MUSIC(MUSIC_282)
OBJECT_EFFECT(OBJECT_LOCALID(LOCALID_NEO_GHOUL_1), OBJECT_EFFECT_TELEPORT)
SHOW_OBJECT(LOCALID_NEO_GHOUL_1, 128, 96, 0, 0, 0)
DELAY(16)
TALK(PORTRAIT_ARCANA, EXPRESSION_NEUTRAL, PORTRAIT_LEFT,
    "Hyahahahaha! Nice attack...\n\n"
    "But how much longer can you\n"
    "keep this up?\n\n"
    "Go!\n\n"
)
HIDE_PORTRAIT()
DELAY(16)
WALK_OBJECT_Y(LOCALID_NEO_GHOUL_2, 24)
MOVE_OBJECT(LOCALID_NEO_GHOUL_2, DIRECTION_RIGHT, 0, 0)
SET_OBJECT_POSITION(LOCALID_NEO_GHOUL_1, 0, 52, 0)
WALK_OBJECT_X(LOCALID_NEO_GHOUL_1, 24)
WALK_OBJECT_Y(LOCALID_NEO_GHOUL_1, 32)
WALK_OBJECT_X(LOCALID_NEO_GHOUL_1, 84)
DELAY(16)
TALK(PORTRAIT_MAI, EXPRESSION_MAI_ANGRY, PORTRAIT_LEFT,
    "Grr... There's too many of\n"
    "them...\n\n"
)
HIDE_PORTRAIT()
DELAY(16)
WALK_OBJECT_Y(0, 70)
WALK_OBJECT_X(0, 21)
WALK_OBJECT_Y(0, 30)
WALK_OBJECT_X(0, 40)
WALK_OBJECT_Y(13, 70)
WALK_OBJECT_X(13, 21)
WALK_OBJECT_Y(13, 36)
WALK_OBJECT_X(13, 30)
WALK_OBJECT_Y(14, 70)
WALK_OBJECT_X(14, 21)
WALK_OBJECT_Y(14, 36)
WALK_OBJECT_X(14, 30)
WALK_OBJECT_Y(14, 24)
WALK_OBJECT_X(14, 30)
DELAY(8)
PLAY_MUSIC(MUSIC_271)
REACTION(REACTION_EXCLAMATION_MARK, SPRITE_ARKANA)
DELAY(8)
PLAY_MUSIC(MUSIC_407)
MOVE_OBJECT(SPRITE_ARKANA, DIRECTION_LEFT, 0, 0)
MOVE_OBJECT(LOCALID_NEO_GHOUL_1, DIRECTION_LEFT, 0, 0)
MOVE_OBJECT(LOCALID_NEO_GHOUL_2, DIRECTION_LEFT, 0, 0)
DELAY(16)
TALK(PORTRAIT_ARCANA, EXPRESSION_NEUTRAL, PORTRAIT_LEFT,
    "Hyahahahaha! How good of you\n"
    "to visit!\n\n"
    "What's the fun without a\n"
    "captive audience!\n\n"
    "I'll give you two choices.\n\n"
    "Do nothing but watch this\n"
    "stunning show as I defeat\n\n"
    "this woman...\n\n"
    "Or step in and be magically\n"
    "defeated by me!\n\n"
    "Hyahahahahaha!\n\n"
    "Whichever choice you make,\n"
    "I win!\n\n"
    "Hyahahahaha!\n\n"
)
PORTRAIT(PORTRAIT_NONE, EXPRESSION_NEUTRAL, PORTRAIT_LEFT)
TALK(PORTRAIT_ARCANA, EXPRESSION_NEUTRAL, PORTRAIT_LEFT,
    "Are you that anxious to be\n"
    "humiliated?!\n\n"
    "I will have you begging on\n"
    "your knees before it's\n\n"
    "over!\n\n"
    "I, Arkana, accept your\n"
    "challenge!\n\n"
)
DUEL(DUELIST_ARCANA_156)
PLAY_MUSIC(MUSIC_407)
TALK(PORTRAIT_ARCANA, EXPRESSION_ARKANA_DEFEATED, PORTRAIT_LEFT,
    "Wh-what? I lost? The great\n"
    "Arkana lost?\n\n"
    "Hyahahahaha! This simply\n"
    "cannot be!\n\n"
    "This must be a mistake!\n"
    "Hyahahahaha!\n\n"
)
HIDE_PORTRAIT()
DELAY(16)
WALK_OBJECT_X(SPRITE_ARKANA, 102)
WALK_OBJECT_Y(SPRITE_ARKANA, 68)
WALK_OBJECT_X(SPRITE_ARKANA, 60)
WALK_OBJECT_Y(SPRITE_ARKANA, 96)
WALK_OBJECT_X(LOCALID_NEO_GHOUL_1, 102)
WALK_OBJECT_Y(LOCALID_NEO_GHOUL_1, 68)
WALK_OBJECT_X(LOCALID_NEO_GHOUL_1, 60)
WALK_OBJECT_Y(LOCALID_NEO_GHOUL_1, 96)
WALK_OBJECT_Y(LOCALID_NEO_GHOUL_2, 32)
WALK_OBJECT_X(LOCALID_NEO_GHOUL_2, 102)
WALK_OBJECT_Y(LOCALID_NEO_GHOUL_2, 68)
WALK_OBJECT_X(LOCALID_NEO_GHOUL_2, 60)
WALK_OBJECT_Y(LOCALID_NEO_GHOUL_2, 96)
HIDE_PORTRAIT()
FADE_MUSIC(2)
DELAY(32)
WALK_OBJECT_Y(SPRITE_MAI, 32)
WALK_OBJECT_X(SPRITE_MAI, 72)
DELAY(16)
TALK(PORTRAIT_MAI, EXPRESSION_NEUTRAL, PORTRAIT_LEFT,
    "You came here to save me?\n\n"
)
PORTRAIT(PORTRAIT_NONE, EXPRESSION_NEUTRAL, PORTRAIT_LEFT)
TALK(PORTRAIT_MAI, EXPRESSION_NEUTRAL, PORTRAIT_LEFT,
    "Thanks. I couldn't have done\n"
    "it without you.\n\n"
)
TEXT
(
    "So what exactly brings you\n"
    "out here?\n\n"
    "Did you want to see me about\n"
    "something?\n\n"
)
PLAY_MUSIC(MUSIC_332)
FADE_SCREEN(4)
PLAY_MUSIC(MUSIC_702)
TALK(PORTRAIT_MAI, EXPRESSION_MAI_ANGRY, PORTRAIT_LEFT,
    "What! Those creeps...\n\n"
    "The Neo Ghouls are\n"
    "destroying the city?\n\n"
    "I won't let those cheaters\n"
    "get away with it!\n\n"
    "All right, I'll help.\n\n"
    "We'll show them what a\n"
    "real duelist is all about!\n\n"
)
HIDE_PORTRAIT()
DELAY(16)
WALK_OBJECT_X(SPRITE_MAI, 102)
WALK_OBJECT_Y(SPRITE_MAI, 68)
WALK_OBJECT_X(SPRITE_MAI, 60)
WALK_OBJECT_Y(SPRITE_MAI, 96)
SET_FLAG(EVENT_FLAG_DEFEATED_ARKANA)
CONDITION_CHECK(0)
TALK(PORTRAIT_YUGI, EXPRESSION_NEUTRAL, PORTRAIT_RIGHT,
    "Okay! Let's go find another\n"
    "ally!\n\n"
)
HIDE_PORTRAIT()
FADE_MUSIC(2)
DELAY(32)

EVENT_SCRIPT_REPLACEMENT(0x08EA3D40, map_26_state_06_08EA3D40, 0x08F04040, 0x08F04040)
TALK(PORTRAIT_YUGI, EXPRESSION_NEUTRAL, PORTRAIT_RIGHT,
    "Okay! We should also get\n"
    "back to the Town of Domino!\n\n"
)
  HIDE_PORTRAIT()
  FADE_MUSIC(2)
  DELAY(32)
  SET_FLAG(EVENT_FLAG_FOUND_ALL_ALLIES)
  FALLTHROUGH()
END_EVENT_SCRIPT()

EVENT_SCRIPT_REPLACEMENT(0x08EA3D28, map_26_state_06_08EA3D28, 0x08EA3D34, 0x08F04040)
TALK(PORTRAIT_MAI, EXPRESSION_NEUTRAL, PORTRAIT_LEFT,
    "Well, I didn't really need\n"
    "your help, anyway.\n\n"
    "I could've beaten the Neo\n"
    "Ghouls by myself!\n\n"
)
  FALLTHROUGH()
END_EVENT_SCRIPT()

EVENT_SCRIPT_REPLACEMENT(0x08EA3D04, map_26_state_06_08EA3D04, 0x08EA3D10, 0x08F0404C)
TALK(PORTRAIT_ARCANA, EXPRESSION_NEUTRAL, PORTRAIT_LEFT,
    "You must be joking.\n\n"
    "You're demanding that I beg\n"
    "you for forgiveness?\n\n"
    "Hyahahahaha! Whatever!\n\n"
    "You're about to witness my\n"
    "dazzling duel skills!\n\n"
    "Prepare for Arkana's most\n"
    "exhilarating performance\n\n"
    "yet!\n\n"
)
  DUEL(DUELIST_ARCANA_156)
  FALLTHROUGH()
END_EVENT_SCRIPT()

EVENT_SCRIPT_REPLACEMENT(0x08EA3D58, map_26_state_06_08EA3D58, 0x08F04040, 0x08F04040)
TEXT
  (
      "The Neo Ghouls are\n"
      "frightening individuals\n\n"
      "indeed.\n\n"
      "If it weren't for Mai, my\n"
      "rare cards would be gone.\n\n"
  )
  FALLTHROUGH()
END_EVENT_SCRIPT()

EVENT_SCRIPT_REPLACEMENT(0x08EA3D64, map_26_state_06_08EA3D64, 0x08F04040, 0x08F04040)
TEXT
  (
      "Oh, no. No more duels for\n"
      "me, thank you kindly.\n\n"
  )
  FALLTHROUGH()
END_EVENT_SCRIPT()

EVENT_SCRIPT_REPLACEMENT(0x08EA3D70, map_26_state_06_08EA3D70, 0x08F04040, 0x08F04040)
TEXT
  (
      "How could we be attacked by\n"
      "the Neo Ghouls?\n\n"
      "Dear me, this ship's\n"
      "security is not up to\n\n"
      "snuff.\n\n"
  )
  FALLTHROUGH()
END_EVENT_SCRIPT()

EVENT_SCRIPT_REPLACEMENT(0x08EA3D7C, map_26_state_06_08EA3D7C, 0x08F04040, 0x08F04040)
TEXT
  (
      "I'm sorry, the idea doesn't\n"
      "enthuse me.\n\n"
  )
  FALLTHROUGH()
END_EVENT_SCRIPT()

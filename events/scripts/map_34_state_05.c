#include "event_macros.h"
#include "overworld.h"
#include "constants/music_ids.h"

/* map_34_state_05: map 34 state 5 */

EVENT_SCRIPT_REPLACEMENT(0x08EC26C8, map_34_state_05_08EC26C8, EVENT_NOP, EVENT_NOP)
  LOAD_SPRITE(1, SPRITE_SERIOUS_ARCHEOLOGIST)
  LOAD_SPRITE(2, SPRITE_CHIEF_ARCHEOLOGIST)
  LOAD_SPRITE(3, SPRITE_FRIVOULOUS_ARCHEOLOGIST)
  PLAY_MUSIC(MUSIC_707)
  PLAY_MUSIC(MUSIC_381)
  SHOW_OBJECT(1, 106, 32, 0, 0, 0)
  DELAY(8)
  MOVE_OBJECT(1, DIRECTION_DOWN, 4, 0)
  DELAY(8)
  PLAY_MUSIC(MUSIC_293)
  OBJECT_EFFECT(OBJECT_1, OBJECT_EFFECT_JUMPING)
  DELAY(8)
  TEXT("This is a huge discovery, eh!")
  HIDE_PORTRAIT()
  DELAY(8)
  PLAY_MUSIC(MUSIC_271)
  REACTION(REACTION_EXCLAMATION_MARK, OBJECT_2 | OBJECT_3)
  DELAY(8)
  MOVE_OBJECT(2, DIRECTION_RIGHT, 0, 0)
  MOVE_OBJECT(3, DIRECTION_RIGHT, 0, 0)
  DELAY(8)
  MOVE_OBJECT(1, DIRECTION_UP, 4, 0)
  SHOW_OBJECT(1, 128, 96, 0, 0, 0)
  WALK_OBJECT_Y(3, 64)
  WALK_OBJECT_X(3, 60)
  WALK_OBJECT_Y(3, 58)
  WALK_OBJECT_X(3, 92)
  WALK_OBJECT_Y(3, 48)
  MOVE_OBJECT(2, DIRECTION_DOWN, 0, 0)
  DELAY(8)
  PLAY_MUSIC(MUSIC_293)
  OBJECT_EFFECT(OBJECT_2, OBJECT_EFFECT_JUMPING)
  DELAY(8)
  TEXT("We ought to hurry too.")
  HIDE_PORTRAIT()
  DELAY(8)
  WALK_OBJECT_X(2, 106)
  WALK_OBJECT_Y(2, 32)
  SHOW_OBJECT(2, 128, 96, 0, 0, 0)
  DELAY(16)
  WALK_OBJECT_X(3, 106)
  WALK_OBJECT_Y(3, 40)
  DELAY(16)
  MOVE_OBJECT(3, DIRECTION_DOWN, 0, 0)
  DELAY(16)
  TEXT("Maybe our find will really make us famous!")
  HIDE_PORTRAIT()
  DELAY(16)
  PLAY_MUSIC(MUSIC_273)
  REACTION(REACTION_LOVE_HEART, OBJECT_3)
  WALK_OBJECT_Y(3, 32)
  SHOW_OBJECT(3, 128, 96, 0, 0, 0)
  FADE_MUSIC(8)
  FADE_SCREEN(8)
  WARP(LOCATION_CANADA_RUINS_MILLENNIUM_GUARDIAN, 3, 0, 0)
  FALLTHROUGH()
END_EVENT_SCRIPT()

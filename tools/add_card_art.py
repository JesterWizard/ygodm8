...
def custom_card_qty_bytes_for(custom_cards: int) -> int:
    """Qty buffer size: floor 0x200, then multiple of 4.

    Odd sizes flip u16 trunk buffer parity (5×qty). Sizes that are 2-mod-4 leave the
    expanded SortableEntry array (u64 fields) misaligned. Aligning to 4 avoids both.
    """
    size = max(custom_cards, 0x200)
    return (size + 3) & ~3
...
def render_card_memory_sizes_asm(manifest: dict) -> str:
    ...
    # ponytail: agbcc lays out SortableEntry as 12 bytes (u16 + u64 @4); vanilla buffer is 800×12.
    ...
    return "\n".join(
        [
            ...
            f".equ SORTABLE_ENTRIES_BYTES, 0x{sortable_entries_bytes:04X}",
            "",
        ]
    )

...
KEEP_LIST_JSON = Path(__file__).resolve().parent / "custom_card_keep_list.json"
CUSTOM_CARD_START_CONST = "SORCERER_OF_DARK_MAGIC"
EFFECT_SOURCE_DIRS = (
    ...
    "src_custom/turn_effects",
    "src_custom/card_passives",
)
...
def keep_list_enabled() -> bool:
    value = os.environ.get("YGO_CUSTOM_CARD_KEEP_LIST", "1")
    return value not in ("0", "false", "False", "no", "NO")


def load_keep_list_spec(path: Path | str | None = None) -> dict:
    path = Path(path or KEEP_LIST_JSON)
    return json.loads(path.read_text(encoding="utf-8"))


def manifest_custom_start_index(manifest: dict) -> int:
    return next(
        (
            index
            for index, item in enumerate(manifest["cards"])
            if item["card_const"] == CUSTOM_CARD_START_CONST
        ),
        len(manifest["cards"]),
    )


def filter_manifest_to_keep_list(manifest: dict, *, enabled: bool | None = None) -> dict:
    if enabled is None:
        enabled = keep_list_enabled()
    if not enabled:
        return manifest

    spec = load_keep_list_spec()
    keep_consts = set(spec.get("keep_consts") or [])
    cards = manifest["cards"]
    custom_start = manifest_custom_start_index(manifest)
    filtered_custom = [item for item in cards[custom_start:] if item["card_const"] in keep_consts]
    missing = keep_consts - {item["card_const"] for item in cards[custom_start:]}
    if missing:
        sample = ", ".join(sorted(missing)[:8])
        raise ManifestValidationError(f"keep_consts not in manifest: {sample}")

    return {"cards": cards[:custom_start] + filtered_custom}


def effect_stem_candidates(card_const: str) -> set[str]:
    stem = card_const.lower()
    stems = {stem}
    if stem.startswith("the_"):
        stems.add(stem[4:])
    else:
        stems.add(f"the_{stem}")
    return stems


def find_effect_sources_for_const(card_const: str, root: Path | None = None) -> list[Path]:
    root = root or ROOT
    found: list[Path] = []
    for stem in effect_stem_candidates(card_const):
        for rel_dir in EFFECT_SOURCE_DIRS:
            path = root / rel_dir / f"{stem}.c"
            if path.is_file():
                found.append(path)
    return found


def collect_excluded_effect_sources(full_manifest: dict, filtered_manifest: dict, root: Path | None = None) -> list[Path]:
    root = root or ROOT
    custom_start = manifest_custom_start_index(full_manifest)
    kept = {item["card_const"] for item in filtered_manifest["cards"][custom_start:]}
    excluded: list[Path] = []
    for item in full_manifest["cards"][custom_start:]:
        if item["card_const"] not in kept:
            excluded.extend(find_effect_sources_for_const(item["card_const"], root))
    return sorted(set(excluded))


def render_custom_card_keep_excludes_mk(excluded: list[Path], root: Path | None = None) -> str:
    root = root or ROOT
    rel_paths = [str(path.relative_to(root)).replace("\\", "/") for path in excluded]
    lines = ["# Generated by tools/add_card_art.py. Do not edit.", "CUSTOM_CARD_KEEP_EXCLUDE_SRCS :="]
    if rel_paths:
        lines[1] += " \\"
        lines.extend(f"  {rel_path} \\" for rel_path in rel_paths)
    lines.append("")
    return "\n".join(lines)


def write_custom_card_keep_excludes_mk(
    path: Path | str,
    full_manifest: dict,
    *,
    ...
    if not enabled:
        path.write_text(
            "# Custom card keep list disabled.\nCUSTOM_CARD_KEEP_EXCLUDE_SRCS :=\n",
            encoding="utf-8",
        )
        return

    filtered = filter_manifest_to_keep_list(full_manifest, enabled=True)
    excluded = collect_excluded_effect_sources(full_manifest, filtered, root)
    path.write_text(render_custom_card_keep_excludes_mk(excluded, root), encoding="utf-8")


def removed_custom_card_consts(full_manifest: dict, filtered_manifest: dict) -> list[str]:
    custom_start = manifest_custom_start_index(full_manifest)
    kept = {item["card_const"] for item in filtered_manifest["cards"][custom_start:]}
    return [
        item["card_const"]
...
def render_removed_card_stub_header(full_manifest: dict, filtered_manifest: dict) -> str:
    removed = removed_custom_card_consts(full_manifest, filtered_manifest)
    lines = [
    ...
    if removed:
        lines.append("")
        for index, card_const in enumerate(removed):
            lines.append(f"#define {card_const} (0xE000 + {index})")
    lines.extend(["", "#endif // GUARD_CUSTOM_CARD_REMOVED_STUBS_H", ""])
    return "\n".join(lines)

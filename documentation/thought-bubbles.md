...
Each bubble lives in its own folder under `src/hooks/assets/thought_bubbles/`.

Expected layout:

```text
src/hooks/assets/thought_bubbles/
  event_01/
    thought.png
  event_02/
    thought.png
...
- `src/hooks/thought_bubble_table.inc`

It exposes two lists:

```c
#define THOUGHT_BUBBLE_ASSET_LIST(_) \
  _(DEFAULT, "src/hooks/assets/thought_bubbles/event_01/thought.dmp", "src/hooks/assets/thought_bubbles/event_01/thought.gbapal") \
  _(MILLENNIUM_NECKLACE, "src/hooks/assets/thought_bubbles/event_02/thought.dmp", "src/hooks/assets/thought_bubbles/event_02/thought.gbapal")
...
|--------|----------|-------------|
| Bubble asset folders | `src/hooks/assets/thought_bubbles/` | Holds one `thought.png` per event bubble folder |
| Bubble build rules | `graphics.mk` | Recursively generates `thought.dmp` and `thought.gbapal` for every thought bubble folder |
| Bubble registration table | `src/hooks/thought_bubble_table.inc` | Single source of truth for bubble ids, folders, and event-flag assignments |
| Runtime bubble loader | `LoadThoughtBubbleGfx` in `src/hooks/overworld_hooks.c` | Loads the selected bubble tiles and palette into OBJ VRAM |
| Runtime bubble selection | `GetThoughtBubbleIdForFlag` in `src/hooks/overworld_hooks.c` | Converts the most recent event flag into a bubble id |
| Latest-flag tracking | `SetFlag__Replacement` in `src/hooks/flag_hooks.c` | Records the newest event flag that has been triggered |
| Reset behavior | `InitFlags__Replacement` in `src/hooks/flag_hooks.c` | Clears the tracked latest flag when flags are reinitialized |
| Public accessor | `include/thought_bubble.h` | Exposes the latest thought-bubble event flag to the overworld hook |

## TODO

- Replace raw numeric flags in `src/hooks/thought_bubble_table.inc` with named constants if a dedicated event-flag header is added.

...
static void ResolveTarget(u8 fixedRow, u8 fixedCol)
{
  ...
  zone->unk4 += 1; /* Venom Counter */
  if (self != NULL) {
    self->unk4 |= DUEL_CANNOT_ATTACK_THIS_TURN_MARK; /* cannot attack this turn */
...
unsigned char CanActivateVENOM_SNAKE(void)
{
  ...
  if (zone == NULL || zone->id != VENOM_SNAKE)
    return FALSE;
...
{
  Duel_ShowEffectTextTyped(VENOM_SNAKE, 2);

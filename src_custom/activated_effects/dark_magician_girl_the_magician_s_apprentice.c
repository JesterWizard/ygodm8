...
void TryDarkMagicianGirlTheMagicianSApprenticeOnMonsterPlacement(struct DuelCard *zone)
{
  ...
  Duel_ShowEffectTextTyped(DARK_MAGICIAN_GIRL_THE_MAGICIAN_S_APPRENTICE, 8);

  if (Duel_AddDeckCardToHand(turnDuelist, SHINING_SARCOPHAGUS, TRUE) != DUEL_ACTION_OK)
    return;

  EffectOpt_MarkUsed(DARK_MAGICIAN_GIRL_THE_MAGICIAN_S_APPRENTICE);
...
void ActivateDARK_MAGICIAN_GIRL_THE_MAGICIAN_S_APPRENTICEEffect(void)
{
  ...
  Duel_ShowEffectTextTyped(DARK_MAGICIAN_GIRL_THE_MAGICIAN_S_APPRENTICE, 2);
...
u8 CanSpecialSummonDarkMagicianGirlTheMagiciansApprenticeFromHand(u8 handZone)
{
...
u8 TrySpecialSummonDarkMagicianGirlTheMagiciansApprenticeFromHand(u8 handZone)
{
  struct DuelSummonOpts opts = Duel_DefaultSpecialSummonOpts(TRUE);

  if (!CanSpecialSummonDarkMagicianGirlTheMagiciansApprenticeFromHand(handZone))
    return FALSE;

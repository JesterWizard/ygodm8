...
#define AQUA_DOLPHIN_DAMAGE 500
...
unsigned char CanActivateNEO_SPACIAN_AQUA_DOLPHIN(void)
{
  ...
  if (zone == NULL || zone->id != NEO_SPACIAN_AQUA_DOLPHIN)
    return FALSE;
...
void ActivateNEO_SPACIAN_AQUA_DOLPHINEffect(void)
{
  ...
  Duel_ShowEffectTextTyped(NEO_SPACIAN_AQUA_DOLPHIN, 2);
  ...
  if (ownAtk >= chosenAtk) {
    ...
    if (Duel_ChangeLp(INACTIVE_DUELIST, -(s32)AQUA_DOLPHIN_DAMAGE, TRUE) == DUEL_ACTION_DUEL_OVER)
      return;
  } else {
    if (Duel_ChangeLp(ACTIVE_DUELIST, -(s32)AQUA_DOLPHIN_DAMAGE, TRUE) == DUEL_ACTION_DUEL_OVER)
      return;
  }

...
static const u8 sCreatorPickLabels[] APPEND_RODATA = {
...
static u8 IsGyMonster(u16 cardId)
{
...
static u8 GyHasMonster(u8 fixedDuelist)
{
  ...
  if (!GraveyardExpand_IsEnabled())
    return IsGyMonster(gDuel.duelistbattleState[fixedDuelist].graveyard);

  for (i = 0; i < GraveyardExpand_GetCount(fixedDuelist); i++) {
    if (IsGyMonster(GraveyardExpand_GetCardAt(fixedDuelist, i)))
      return TRUE;
  }
...
static u8 LoadGyMonsterMenu(u8 fixedDuelist, u8 *gyIndexMap)
{
...
static s8 FindBestGyMonsterIndex(u8 fixedDuelist)
{
  u8 gyCount = GraveyardExpand_GetCount(fixedDuelist);
  u8 bestIndex = 0xFF;
  u16 bestAtk = 0;
...
static s8 PlayerPickGyMonsterIndex(u8 fixedDuelist)
{
  ...
  menuCount = LoadGyMonsterMenu(fixedDuelist, gyIndexMap);
  if (menuCount == 0) {
    DECKMENU_RESTORE();
    return -1;
  }
  ...
  if (!DeckMenuMainPickConfirmWithLabels(sCreatorPickLabels,
                                         ARRAY_COUNT(sCreatorPickLabels))) {
    DECKMENU_RESTORE();
...
unsigned char CanActivateTHE_CREATOR(void)
{
  ...
  if (zone == NULL || zone->id != THE_CREATOR)
    return FALSE;
  ...
  return GyHasMonster(fixedDuelist);
}
...
void ActivateTHE_CREATOREffect(void)
{
  ...
  if (WhoseTurn() == DUEL_PLAYER && GraveyardExpand_IsEnabled())
    gyIndex = PlayerPickGyMonsterIndex(fixedDuelist);
  else if (GraveyardExpand_IsEnabled())
    gyIndex = FindBestGyMonsterIndex(fixedDuelist);
  else
    gyIndex = GyHasMonster(fixedDuelist) ? 0 : -1;
  ...
  if (SpecialSummonFromGyIndex((u8)gyIndex) == DUEL_ACTION_DUEL_OVER)
    return;

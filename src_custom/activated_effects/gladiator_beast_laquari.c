...
static void ShuffleSelfTagOut(struct DuelCard *self)
{
  ...
  tagId = FindOtherGladiatorBeastInDeck(GLADIATOR_BEAST_LAQUARI);
  if (tagId == CARD_NONE)
    return;
...
unsigned char CanActivateGLADIATOR_BEAST_LAQUARI(void)
{
  ...
  return IsOtherGladiatorBeastInDeck(GLADIATOR_BEAST_LAQUARI);
}
...
void ActivateGLADIATOR_BEAST_LAQUARIEffect(void)
{
  ...
  Duel_ShowEffectTextTyped(GLADIATOR_BEAST_LAQUARI, 2);

...
unsigned char CanActivateCHRYSALIS_DOLPHIN(void)
{
  ...
  if (zone == NULL || zone->id != CHRYSALIS_DOLPHIN)
    return FALSE;
...
void ActivateCHRYSALIS_DOLPHINEffect(void)
{
  ...
  Duel_ShowEffectTextTyped(CHRYSALIS_DOLPHIN, 2);

  if (self == NULL || !NeoSpaceOnField() || IsDuelOver() == TRUE)
    return;

  /* Tribute self, then SS Neo-Spacian Aqua Dolphin from hand or Deck. */
  ClearZone(self);
  ...
  if (Duel_SpecialSummonFromHand(ACTIVE_DUELIST, NEO_SPACIAN_AQUA_DOLPHIN, NULL, opts) != DUEL_ACTION_OK)
    Duel_SpecialSummonFromDeck(ACTIVE_DUELIST, NEO_SPACIAN_AQUA_DOLPHIN, opts);

...
static u8 CanTagOut(void)
{
  ...
  return DeckHasTwoDifferentGladiatorBeasts(GLADIATOR_BEAST_NEROKIUS);
}
...
static void ShuffleSelfTagOut(struct DuelCard *self)
{
  ...
  /* ponytail: Extra Deck return + battle protection need ED/battle hooks. */
  ClearZone(self);
  ...
  SpecialSummonTwoGladiatorBeastsFromDeck(GLADIATOR_BEAST_NEROKIUS);
}
...
unsigned char CanActivateGLADIATOR_BEAST_NEROKIUS(void)
{
  ...
  if (zone == NULL || zone->id != GLADIATOR_BEAST_NEROKIUS)
    return FALSE;

  /* ponytail: battle indestruct + opp chain block + end-of-BP gate need battle hooks.

...
static u8 CanTagOut(void)
{
  ...
  return IsOtherGladiatorBeastInDeck(GLADIATOR_BEAST_VESPASIUS);
}
...
static void ShuffleSelfTagOut(struct DuelCard *self)
{
  ...
  tagId = FindOtherGladiatorBeastInDeck(GLADIATOR_BEAST_VESPASIUS);
  if (tagId == CARD_NONE)
    return;
...
unsigned char CanActivateGLADIATOR_BEAST_VESPASIUS(void)
{
  ...
  /* ponytail: FromHand battle SS + continuous +500 while GB-SS'd + end-BP battled
   * gate need battle/summon hooks. Ceiling: OPT tag-out shuffle→SS. */
...
void ActivateGLADIATOR_BEAST_VESPASIUSEffect(void)
{
  ...
  Duel_ShowEffectTextTyped(GLADIATOR_BEAST_VESPASIUS, 2);

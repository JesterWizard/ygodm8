...
struct ElementalHeroPoisonRoseActionData {
...
extern struct ElementalHeroPoisonRoseActionData sActionData;
...
void ApplyElementalHeroPoisonRoseBattleEffect(void)
{
  ...
  if (IsPlayerAttackAction(sActionData.id)
      && sActionData.playerCardId == ELEMENTAL_HERO_POISON_ROSE
  ...
  if (IsOpponentAttackAction(sActionData.id)
      && sActionData.opponentCardId == ELEMENTAL_HERO_POISON_ROSE

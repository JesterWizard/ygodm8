...
struct DarkDriceratopsActionData {
...
extern struct DarkDriceratopsActionData sActionData;
...
static void ApplyDarkDriceratopsPiercingDamage(void) {
  u16 attackerAtk;
  u16 defenderDef;
  u16 damage;

  if (sActionData.id == 2 && sActionData.playerCardId == DARK_DRICERATOPS) {
  ...
  if (sActionData.id == 5 && sActionData.opponentCardId == DARK_DRICERATOPS) {
...
void ApplyDarkDriceratopsBattleEffect(void) {
  if (sActionData.playerCardId != DARK_DRICERATOPS && sActionData.opponentCardId != DARK_DRICERATOPS)
    return;
  if (!IsMonsterBattleAction(sActionData.id))
    return;

  ApplyDarkDriceratopsPiercingDamage();
}

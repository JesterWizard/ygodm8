...
struct MadSwordBeastActionData {
...
extern struct MadSwordBeastActionData sActionData;
...
static void ApplyMadSwordBeastPiercingDamage(void) {
  u16 attackerAtk;
  u16 defenderDef;
  u16 damage;

  if (sActionData.id == 2 && sActionData.playerCardId == MAD_SWORD_BEAST) {
  ...
  if (sActionData.id == 5 && sActionData.opponentCardId == MAD_SWORD_BEAST) {
...
void ApplyMadSwordBeastBattleEffect(void) {
  if (sActionData.playerCardId != MAD_SWORD_BEAST && sActionData.opponentCardId != MAD_SWORD_BEAST)
    return;
  if (!IsMonsterBattleAction(sActionData.id))
    return;

  ApplyMadSwordBeastPiercingDamage();
}

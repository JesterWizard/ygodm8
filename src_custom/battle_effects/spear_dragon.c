...
extern struct SpearDragonActionData sActionData;
...
static void ApplySpearDragonPiercingDamage(void) {
  u16 attackerAtk;
  u16 defenderDef;
  u16 damage;

  if (sActionData.id == 2 && sActionData.playerCardId == SPEAR_DRAGON) {
  ...
  if (sActionData.id == 5 && sActionData.opponentCardId == SPEAR_DRAGON) {
...
static void SwitchSpearDragonToDefensePosition(void) {
  struct DuelCard *zone;

  if (sActionData.playerCardId == SPEAR_DRAGON
      && !(sActionData.flags & FLAG_GRAVEYARD_PLAYER)) {
    zone = gFixedZones[sActionData.playerMonsterRow][sActionData.unkA];
    if (zone->id == SPEAR_DRAGON) {
      zone->isDefending = TRUE;
      zone->isFaceUp = TRUE;
    }
  }

  if (sActionData.opponentCardId == SPEAR_DRAGON
...
void ApplySpearDragonBattleEffect(void) {
  if (sActionData.playerCardId != SPEAR_DRAGON && sActionData.opponentCardId != SPEAR_DRAGON)
    return;
  if (!IsMonsterBattleAction(sActionData.id))
    return;

  ApplySpearDragonPiercingDamage();
  SwitchSpearDragonToDefensePosition();
}

...
extern struct PlasmaViceActionData sActionData;
...
void ApplyElementalHeroPlasmaVicePiercingBattleEffect(void) {
  ...
  if (sActionData.playerCardId != ELEMENTAL_HERO_PLASMA_VICE
      && sActionData.opponentCardId != ELEMENTAL_HERO_PLASMA_VICE)
    return;
  if (!IsMonsterBattleAction(sActionData.id))
    return;

  if (sActionData.id == 2 && sActionData.playerCardId == ELEMENTAL_HERO_PLASMA_VICE) {
  ...
  if (sActionData.id == 5 && sActionData.opponentCardId == ELEMENTAL_HERO_PLASMA_VICE) {

...
void BeginBookOfMoonTargeting(u8 originFixedRow, u8 originFixedCol)
{
  ...
  Duel_ShowEffectTextTyped(BOOK_OF_MOON, 1);
...
static void BookOfMoon_ResolveBody(void)
{
...
APPEND_TEXT void EffectBookOfMoon(void)
{
  ...
  if (Duel_TryResolveSpellThroughTraps(BOOK_OF_MOON, BookOfMoon_ResolveBody) == DUEL_ACTION_BLOCKED)
    return;
}

...
u8 CanActivateConfiscation(void)
{
...
static void Confiscation_ResolveBody(void)
{
  if (!CanActivateConfiscation())
    return;
...
APPEND_TEXT void EffectConfiscation(void)
{
  if (Duel_TryResolveSpellThroughTrapsEx(CONFISCATION, CONFISCATION_LP_COST,
                                         Confiscation_ResolveBody) == DUEL_ACTION_BLOCKED)
    return;
}

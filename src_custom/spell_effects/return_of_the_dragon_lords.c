...
static u8 IsEligibleDragonLordsTarget(u16 cardId)
{
  u8 level;
...
static s8 FindFirstEligibleGyIndex(u8 fixedDuelist)
{
  ...
  if (!GraveyardExpand_IsEnabled()) {
    if (IsEligibleDragonLordsTarget(gTurnDuelistBattleState[ACTIVE_DUELIST]->graveyard))
      return 0;
    return -1;
  }
  ...
  for (i = 0; i < gyCount; i++) {
    if (IsEligibleDragonLordsTarget(GraveyardExpand_GetCardAt(fixedDuelist, i)))
      return (s8)i;
  }
...
static u8 CanActivateReturnOfTheDragonLords(void)
{
  ...
  if (!GraveyardExpand_IsEnabled())
    return IsEligibleDragonLordsTarget(gTurnDuelistBattleState[ACTIVE_DUELIST]->graveyard);
...
static u8 LoadEligibleGyMenu(u8 fixedDuelist, u8 *gyIndexMap)
{
  ...
  for (i = 0; i < gyCount; i++) {
    ...
    if (!IsEligibleDragonLordsTarget(cardId))
      continue;
...
static s8 PlayerPickGyIndex(u8 fixedDuelist)
{
  ...
  if (!DeckMenuMainPickConfirmWithLabels(sReturnDragonLordsPickLabels,
                                         ARRAY_COUNT(sReturnDragonLordsPickLabels))) {
    DECKMENU_RESTORE();
...
static void RETURN_OF_THE_DRAGON_LORDS_ResolveBody(void)
{
  ...
  if (IsDuelOver() == TRUE || !CanActivateReturnOfTheDragonLords())
    return;
...
{
  if (!CanActivateReturnOfTheDragonLords()) {
    if (!gHideEffectText)
      PlayMusic(SFX_FORBIDDEN);
    return;
  }

  if (Duel_TryResolveSpellThroughTraps(RETURN_OF_THE_DRAGON_LORDS,
                                       RETURN_OF_THE_DRAGON_LORDS_ResolveBody)
      == DUEL_ACTION_BLOCKED)
    return;
}

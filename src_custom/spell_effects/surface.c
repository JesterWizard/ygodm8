...
#define SURFACE_MAX_LEVEL 3

static const u8 sSurfacePickLabels[] APPEND_RODATA = {
...
static u8 IsSurfaceType(u16 cardId)
{
...
static u8 IsEligibleSurfaceTarget(u16 cardId)
{
  if (!IsSurfaceType(cardId))
    return FALSE;

  SetCardInfo(cardId);
  return gCardInfo.level <= SURFACE_MAX_LEVEL;
}
...
static s8 FindFirstEligibleGyIndex(u8 fixedDuelist)
{
  ...
  if (!GraveyardExpand_IsEnabled()) {
    if (IsEligibleSurfaceTarget(gTurnDuelistBattleState[ACTIVE_DUELIST]->graveyard))
      return 0;
    return -1;
  }
  ...
  for (i = 0; i < gyCount; i++) {
    if (IsEligibleSurfaceTarget(GraveyardExpand_GetCardAt(fixedDuelist, i)))
      return (s8)i;
  }
...
static u8 CanActivateSurface(void)
{
  ...
  if (!GraveyardExpand_IsEnabled())
    return IsEligibleSurfaceTarget(gTurnDuelistBattleState[ACTIVE_DUELIST]->graveyard);
...
static u8 LoadEligibleGyMenu(u8 fixedDuelist, u8 *gyIndexMap)
{
  ...
  for (i = 0; i < gyCount; i++) {
    ...
    if (!IsEligibleSurfaceTarget(cardId))
      continue;
...
static s8 PlayerPickGyIndex(u8 fixedDuelist)
{
  ...
  if (!DeckMenuMainPickConfirmWithLabels(sSurfacePickLabels, ARRAY_COUNT(sSurfacePickLabels))) {
    DECKMENU_RESTORE();
...
static void SURFACE_ResolveBody(void)
{
  struct DuelCard *spellZone = gTurnZones[gSpellEffectData.row1][gSpellEffectData.col1];
  u8 fixedDuelist = FixedDuelistForTurnDuelist(ACTIVE_DUELIST);
  s8 gyIndex;

  Duel_ShowEffectText(SURFACE);

  if (IsDuelOver() == TRUE || !CanActivateSurface())
    return;
...
APPEND_TEXT void EffectSURFACE(void)
{
  if (!CanActivateSurface()) {
    if (!gHideEffectText)
      PlayMusic(SFX_FORBIDDEN);
    return;
  }

  if (Duel_TryResolveSpellThroughTraps(SURFACE, SURFACE_ResolveBody) == DUEL_ACTION_BLOCKED)
    return;
}

...
static u8 RecipeIsDarkMachineFusion(const struct FusionRecipe *recipe)
{
  if (recipe == NULL)
    return FALSE;
  ...
  return gCardInfo.attribute == ATTRIBUTE_SHADOW
      && Duel_CardHasMonsterType(recipe->result, TYPE_MACHINE);
}

static void RunPlayerOverloadFusionFlow(void)
{
  ...
  Duel_ShowEffectText(OVERLOAD_FUSION);
  if (IsDuelOver() == TRUE)
    return;

  recipe = FusionDuel_PlayerConfirmFusionPick(feasibleIndices, feasibleCount);
  if (recipe != NULL)
    FusionDuel_ExecuteMiracleFusion(recipe, sources, sourceCount, OVERLOAD_FUSION, FALSE);
}

APPEND_TEXT void EffectOverloadFusion(void)
{
  if (WhoseTurn() != DUEL_PLAYER) {
    ...
    bestIdx = FusionDuel_AiPickBestRecipeIndex(sources, sourceCount, RecipeIsDarkMachineFusion);
    if (bestIdx < 0) {
      if (!gHideEffectText)
        PlayMusic(SFX_FORBIDDEN);
      return;
    ...
    FusionDuel_ExecuteMiracleFusion(&gFusionRecipes[bestIdx], sources, sourceCount,
                                    OVERLOAD_FUSION, TRUE);
    return;
  }

  RunPlayerOverloadFusionFlow();
}

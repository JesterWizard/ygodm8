...
unsigned char ShouldActivateLEGENDARY_MAJU_GARZETT(void)
{
  ...
  if (gActiveEffect.cardId != LEGENDARY_MAJU_GARZETT)
    return FALSE;

  if (GetPendingTributeSummonCardId() != LEGENDARY_MAJU_GARZETT)
    return FALSE;
...
void ActivateLEGENDARY_MAJU_GARZETT(void)
{
  ...
  Duel_ShowEffectTextTyped(LEGENDARY_MAJU_GARZETT, 8);

...
unsigned char ShouldActivateTHE_AGENT_OF_WISDOM_MERCURY(void)
{
  ...
  if (gActiveEffect.cardId != THE_AGENT_OF_WISDOM_MERCURY)
    return FALSE;

  if (GetPendingTributeSummonCardId() != THE_AGENT_OF_WISDOM_MERCURY)
    return FALSE;
...
void ActivateTHE_AGENT_OF_WISDOM_MERCURY(void)
{
  ...
  Duel_ShowEffectTextTyped(THE_AGENT_OF_WISDOM_MERCURY, 8);

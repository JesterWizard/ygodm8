...
unsigned char ShouldActivateLIGHT_AND_DARKNESS_DRAGON(void)
{
  ...
  if (gActiveEffect.cardId != LIGHT_AND_DARKNESS_DRAGON)
    return FALSE;

  if (GetPendingTributeSummonCardId() != LIGHT_AND_DARKNESS_DRAGON)
    return FALSE;
...
void ActivateLIGHT_AND_DARKNESS_DRAGON(void)
{
  ...
  Duel_ShowEffectTextTyped(LIGHT_AND_DARKNESS_DRAGON, 8);

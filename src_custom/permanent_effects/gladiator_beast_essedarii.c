...
unsigned char ShouldActivateGLADIATOR_BEAST_ESSEDARII(void)
{
  ...
  if (gActiveEffect.cardId != GLADIATOR_BEAST_ESSEDARII)
    return FALSE;

  if (GetPendingTributeSummonCardId() != GLADIATOR_BEAST_ESSEDARII)
    return FALSE;
...
void ActivateGLADIATOR_BEAST_ESSEDARII(void)
{
  ...
  Duel_ShowEffectTextTyped(GLADIATOR_BEAST_ESSEDARII, 8);

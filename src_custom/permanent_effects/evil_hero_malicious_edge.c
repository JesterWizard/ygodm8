...
unsigned char ShouldActivateEVIL_HERO_MALICIOUS_EDGE(void)
{
  ...
  if (gActiveEffect.cardId != EVIL_HERO_MALICIOUS_EDGE)
    return FALSE;

  if (GetPendingTributeSummonCardId() != EVIL_HERO_MALICIOUS_EDGE)
    return FALSE;
...
void ActivateEVIL_HERO_MALICIOUS_EDGE(void)
{
  ...
  Duel_ShowEffectTextTyped(EVIL_HERO_MALICIOUS_EDGE, 8);

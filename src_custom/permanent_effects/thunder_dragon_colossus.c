...
unsigned char ShouldActivateTHUNDER_DRAGON_COLOSSUS(void)
{
  ...
  if (gActiveEffect.cardId != THUNDER_DRAGON_COLOSSUS)
    return FALSE;

  if (GetPendingTributeSummonCardId() != THUNDER_DRAGON_COLOSSUS)
    return FALSE;
...
void ActivateTHUNDER_DRAGON_COLOSSUS(void)
{
  ...
  Duel_ShowEffectTextTyped(THUNDER_DRAGON_COLOSSUS, 8);

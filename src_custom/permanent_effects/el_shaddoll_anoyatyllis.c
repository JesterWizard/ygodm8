...
static u8 IsAnoyatyllisGraveyardTrigger(void)
{
  u8 turnDuelist;
  ...
  if (gActiveEffect.cardId != EL_SHADDOLL_ANOYATYLLIS)
    return FALSE;

  turnDuelist = TurnDuelistFromGraveyardRow(gActiveEffect.turnRow);
  if (gActiveEffect.turnRow == 7) {
    if (gTurnDuelistBattleState[INACTIVE_DUELIST]->graveyard != EL_SHADDOLL_ANOYATYLLIS)
      return FALSE;
  } else if (gTurnDuelistBattleState[ACTIVE_DUELIST]->graveyard != EL_SHADDOLL_ANOYATYLLIS) {
    return FALSE;
  }
...
void ActivateEL_SHADDOLL_ANOYATYLLIS(void)
{
  ...
  Duel_ShowEffectTextTyped(EL_SHADDOLL_ANOYATYLLIS, 8);
  if (IsDuelOver() == TRUE)
    return;

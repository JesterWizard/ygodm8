...
unsigned char ShouldActivateGLADIATOR_BEAST_SECUTOR(void)
{
  ...
  if (gActiveEffect.cardId != GLADIATOR_BEAST_SECUTOR)
    return FALSE;

  if (GetPendingTributeSummonCardId() != GLADIATOR_BEAST_SECUTOR)
    return FALSE;
...
void ActivateGLADIATOR_BEAST_SECUTOR(void)
{
  ...
  Duel_ShowEffectTextTyped(GLADIATOR_BEAST_SECUTOR, 8);

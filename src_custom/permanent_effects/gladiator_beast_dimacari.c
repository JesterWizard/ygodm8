...
unsigned char ShouldActivateGLADIATOR_BEAST_DIMACARI(void)
{
  ...
  if (gActiveEffect.cardId != GLADIATOR_BEAST_DIMACARI)
    return FALSE;

  if (GetPendingTributeSummonCardId() != GLADIATOR_BEAST_DIMACARI)
    return FALSE;
...
void ActivateGLADIATOR_BEAST_DIMACARI(void)
{
  ...
  Duel_ShowEffectTextTyped(GLADIATOR_BEAST_DIMACARI, 8);

...
unsigned char ShouldActivateGLADIATOR_BEAST_SAMNITE(void)
{
  ...
  if (gActiveEffect.cardId != GLADIATOR_BEAST_SAMNITE)
    return FALSE;

  if (GetPendingTributeSummonCardId() != GLADIATOR_BEAST_SAMNITE)
    return FALSE;
...
void ActivateGLADIATOR_BEAST_SAMNITE(void)
{
  ...
  Duel_ShowEffectTextTyped(GLADIATOR_BEAST_SAMNITE, 8);

...
unsigned char ShouldActivateEVIL_HERO_INFERNAL_SNIPER(void)
{
  ...
  if (gActiveEffect.cardId != EVIL_HERO_INFERNAL_SNIPER)
    return FALSE;

  if (GetPendingTributeSummonCardId() != EVIL_HERO_INFERNAL_SNIPER)
    return FALSE;
...
void ActivateEVIL_HERO_INFERNAL_SNIPER(void)
{
  ...
  Duel_ShowEffectTextTyped(EVIL_HERO_INFERNAL_SNIPER, 8);

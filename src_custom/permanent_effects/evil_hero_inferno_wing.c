...
unsigned char ShouldActivateEVIL_HERO_INFERNO_WING(void)
{
  ...
  if (gActiveEffect.cardId != EVIL_HERO_INFERNO_WING)
    return FALSE;

  if (GetPendingTributeSummonCardId() != EVIL_HERO_INFERNO_WING)
    return FALSE;
...
void ActivateEVIL_HERO_INFERNO_WING(void)
{
  ...
  Duel_ShowEffectTextTyped(EVIL_HERO_INFERNO_WING, 8);

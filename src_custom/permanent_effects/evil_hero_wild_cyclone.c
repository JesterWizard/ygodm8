...
unsigned char ShouldActivateEVIL_HERO_WILD_CYCLONE(void)
{
  ...
  if (gActiveEffect.cardId != EVIL_HERO_WILD_CYCLONE)
    return FALSE;

  if (GetPendingTributeSummonCardId() != EVIL_HERO_WILD_CYCLONE)
    return FALSE;
...
void ActivateEVIL_HERO_WILD_CYCLONE(void)
{
  ...
  Duel_ShowEffectTextTyped(EVIL_HERO_WILD_CYCLONE, 8);

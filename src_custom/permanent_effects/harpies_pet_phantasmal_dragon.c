...
unsigned char ShouldActivateHARPIES_PET_PHANTASMAL_DRAGON(void)
{
  ...
  if (gActiveEffect.cardId != HARPIES_PET_PHANTASMAL_DRAGON)
    return FALSE;

  if (GetPendingTributeSummonCardId() != HARPIES_PET_PHANTASMAL_DRAGON)
    return FALSE;
...
void ActivateHARPIES_PET_PHANTASMAL_DRAGON(void)
{
  ...
  Duel_ShowEffectTextTyped(HARPIES_PET_PHANTASMAL_DRAGON, 8);

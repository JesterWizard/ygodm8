...
unsigned char ShouldActivateEVIL_HERO_INFERNAL_PRODIGY(void)
{
  ...
  if (gActiveEffect.cardId != EVIL_HERO_INFERNAL_PRODIGY)
    return FALSE;

  if (GetPendingTributeSummonCardId() != EVIL_HERO_INFERNAL_PRODIGY)
    return FALSE;
...
void ActivateEVIL_HERO_INFERNAL_PRODIGY(void)
{
  ...
  Duel_ShowEffectTextTyped(EVIL_HERO_INFERNAL_PRODIGY, 8);

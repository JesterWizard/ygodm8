...
unsigned char ShouldActivateEVIL_HERO_MALICIOUS_FIEND(void)
{
  ...
  if (gActiveEffect.cardId != EVIL_HERO_MALICIOUS_FIEND)
    return FALSE;

  if (GetPendingTributeSummonCardId() != EVIL_HERO_MALICIOUS_FIEND)
    return FALSE;
...
void ActivateEVIL_HERO_MALICIOUS_FIEND(void)
{
  ...
  Duel_ShowEffectTextTyped(EVIL_HERO_MALICIOUS_FIEND, 8);

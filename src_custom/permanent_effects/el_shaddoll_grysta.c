...
static u8 IsGrystaGraveyardTrigger(void)
{
  u8 turnDuelist;
  ...
  if (gActiveEffect.cardId != EL_SHADDOLL_GRYSTA)
    return FALSE;

  turnDuelist = TurnDuelistFromGraveyardRow(gActiveEffect.turnRow);
  if (gActiveEffect.turnRow == 7) {
    if (gTurnDuelistBattleState[INACTIVE_DUELIST]->graveyard != EL_SHADDOLL_GRYSTA)
      return FALSE;
  } else if (gTurnDuelistBattleState[ACTIVE_DUELIST]->graveyard != EL_SHADDOLL_GRYSTA) {
    return FALSE;
  }
...
{
  /* negate opp Special Summon + send Shaddoll from hand need SS chain hook. */
...
void ActivateEL_SHADDOLL_GRYSTA(void)
{
  ...
  Duel_ShowEffectTextTyped(EL_SHADDOLL_GRYSTA, 8);
  if (IsDuelOver() == TRUE)
    return;

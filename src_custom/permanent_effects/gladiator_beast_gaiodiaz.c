...
unsigned char ShouldActivateGLADIATOR_BEAST_GAIODIAZ(void)
{
  ...
  if (gActiveEffect.cardId != GLADIATOR_BEAST_GAIODIAZ)
    return FALSE;

  if (GetPendingTributeSummonCardId() != GLADIATOR_BEAST_GAIODIAZ)
    return FALSE;
...
void ActivateGLADIATOR_BEAST_GAIODIAZ(void)
{
  ...
  Duel_ShowEffectTextTyped(GLADIATOR_BEAST_GAIODIAZ, 8);

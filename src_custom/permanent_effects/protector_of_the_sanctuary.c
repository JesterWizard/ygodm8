...
unsigned char ShouldActivatePROTECTOR_OF_THE_SANCTUARY(void)
{
  ...
  if (gActiveEffect.cardId != PROTECTOR_OF_THE_SANCTUARY)
    return FALSE;

  if (GetPendingTributeSummonCardId() != PROTECTOR_OF_THE_SANCTUARY)
    return FALSE;
...
void ActivatePROTECTOR_OF_THE_SANCTUARY(void)
{
  ...
  Duel_ShowEffectTextTyped(PROTECTOR_OF_THE_SANCTUARY, 8);
